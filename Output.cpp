...
Output::Output()
{
	...
	UI.CellColor_HasCard[0] = FIREBRICK;
	UI.CellColor_HasCard[1] = DARKGREEN;
	UI.CellColor_HasCard[2] = DARKGREEN;
	UI.CellColor_HasCard[3] = FIREBRICK;
	UI.CellColor_HasCard[4] = FIREBRICK;
	UI.CellColor_HasCard[5] = SKYBLUE;
	UI.CellColor_HasCard[6] = DARKGREEN;
	UI.CellColor_HasCard[7] = FIREBRICK;
	UI.CellColor_HasCard[8] = BROWN;
	UI.CellColor_HasCard[9] = ORANGE;
	UI.CellColor_HasCard[10] = BLACK;
	UI.CellColor_HasCard[11] = FIREBRICK;
	//UI.CellColor_HasCard = GOLD;
	UI.CardNumFont = 35;
	...
	UI.PlayerColors[1] = YELLOW;
	UI.PlayerColors[2] = GREEN;
	UI.PlayerColors[3] = BLUE;
...
int Output::GetCellStartX(const CellPosition& cellPos) const
{
	...
	int temp = cellPos.HCell();
	int x = (temp)*UI.CellWidth;
	return x;
...
{
	///TODO: implement the following function as described in Output.h file
	int temp = cellPos.VCell();
	int y = (temp)*UI.CellHeight + UI.ToolBarHeight;
...
void Output::DrawCardNumber(const CellPosition& cellPos, int cardNum) const
{
	...
	if (cardNum <= 12)
	{
		...
		{
			PrintMessage("Out Of The Grid Area");
			pWind->WaitMouseClick(x, y);
		}
	}
	else
	{
		PrintMessage("Invalid Card Number");
		pWind->WaitMouseClick(x, y);
	}
}
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_ADD_LADDER] = "OurImages\\LADDER.jpg";
	MenuItemImages[ITM_ADD_SNAKE] = "OurImages\\SNAKE.jpg";
	MenuItemImages[ITM_ADD_CARD] = "OurImages\\CARD.jpg";
	MenuItemImages[ITM_COPY] = "OurImages\\COPY.jpg";
	MenuItemImages[ITM_CUT] = "OurImages\\CUT.jpg";
	MenuItemImages[ITM_PASTE] = "OurImages\\PASTE.jpg";
	MenuItemImages[ITM_DELETE] = "OurImages\\DELETE.jpg";
	MenuItemImages[ITM_SAVE] = "OurImages\\SAVE.jpg";
	MenuItemImages[ITM_OPEN] = "OurImages\\OPEN.jpg";
	MenuItemImages[ITM_SWITCH_TO_PLAY_MODE] = "OurImages\\GAMEMODE.jpg";
	MenuItemImages[ITM_EXIT1] = "OurImages\\EXIT.jpg";
...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_SWITCH_TO_DESIGN_MODE] = "OurImages\\DESIGNMODE.jpg";
	MenuItemImages[ITM_ROLL_DICE] = "OurImages\\DICE.jpg";
	MenuItemImages[ITM_INPUT_ROLL] = "OurImages\\iDICE.jpg";
	MenuItemImages[ITM_NEW_GAME] = "OurImages\\NEWGAME.jpg";
	MenuItemImages[ITM_EXIT2] = "OurImages\\EXIT.jpg";
	///TODO: Prepare images for each menu item and add it to the list
	// Draw menu item one image at a time
...
void Output::DrawCell(const CellPosition& cellPos, int cardNum) const
{
	...
	if (cardNum == -1)
		...
		pWind->SetBrush(UI.CellColor_HasCard[cardNum-1]);
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.VCell() > toCell.VCell() && fromCell.HCell() == toCell.HCell() && fromCell.VCell() != 0 && !(fromCell.VCell() == 8 && fromCell.HCell() == 0))
	{
		...
		bool exit = false;
		int tempyStart = y1 - UI.LadderYOffset;
		int tempyEnd = y2 + UI.LadderYOffset;
		while (tempyStart >= tempyEnd)
		{
			pWind->SetPen(UI.LadderColor, 6);
			pWind->DrawLine(x12, tempyStart, x34, tempyStart);
			tempyStart -= UI.CellHeight;
		}
	}
}
